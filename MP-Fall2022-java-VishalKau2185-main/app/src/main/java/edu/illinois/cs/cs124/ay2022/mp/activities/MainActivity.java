...
public final class MainActivity extends AppCompatActivity
  ...
  public boolean onQueryTextChange(final String newText) {
    List<Place> result = Place.search(allPlaces, newText);
    if (result.size() == 0) {
      updateShownPlaces(allPlaces);
    } else {
      updateShownPlaces(result);
    }
    return true;
  }
...
//    startActivity(launchAddFavoritePlace);
  ...
  public boolean longPressHelper(final GeoPoint p) {
    Intent launchAddFavoritePlace = new Intent(this, AddPlaceActivity.class);
    String sLat = String.valueOf(p.getLatitude());
    String sLon = String.valueOf(p.getLongitude());

    launchAddFavoritePlace.putExtra("latitude", sLat);
    launchAddFavoritePlace.putExtra("longitude", sLon);
    startActivity(launchAddFavoritePlace);
